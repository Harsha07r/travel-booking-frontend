  ...
  return (
    <>
      <div className="image-container w-80">
        <img src={houseboatTour.image} alt={houseboatTour.title} />
        <h1 className="heading">{houseboatTour.title}</h1>
      </div>

      <Container className="mb-5">
        <h3 className="mb-3">Tour Overview</h3>
        <div className="p-3 bg-light rounded shadow-sm">
          <p>{houseboatTour.overview}</p>
        </div>

        <div className="itinerary-section p-4 mt-4 rounded shadow-sm">
          {houseboatTour.itinerary.map(({ day, desc }, idx) => (
      ...
      <BookingModal
        ...
        tourId={houseboatTour.title}
        tourName={houseboatTour.title}
